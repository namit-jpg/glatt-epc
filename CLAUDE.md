...
- **`CPMEngine.recalculate(projectId)`** — the core engine, modelled on Microsoft Project. Runs the forward/backward pass in **working-day ordinal space** (Mon–Fri, weekends skipped), honoring dependency type + lag with MS Project's start/end-of-day convention (`EF = ES + dur − 1`; the +/−1 boundary adjustment only on FS/SF). **Multiple predecessors** per task: ES = max over all predecessor constraints, pulled from both the `WBS_Dependency__c` junction and the legacy single `Predecessor__c` lookup. **Summary (parent) tasks are not scheduled from their own duration** — their dates are min/max rollups of descendants and they are never flagged critical. Writes `Early/Late Start/Finish`, `Float_Days__c` (= LF − EF), `Is_Critical__c` (leaf && slack ≤ 0); auto-manages `Actual_Start__c`/`Actual_Finish__c` from progress; rolls up budget-weighted progress to `Project__c.Overall_Progress__c`. CPM, actuals, and progress roll-up live in the same class because they touch the same records.
- **`WBSItemTrigger`** (the only trigger) re-runs `CPMEngine` per project on any WBS DML and cleans up orphaned predecessor links on delete; **`TriggerHelper.isRunning`** is the recursion guard — `CPMEngine` sets it before its own DML. `WBS_Dependency__c` edits don't fire this trigger, so the dependency modal calls `runCPM` explicitly after add/remove.
